...
<details>
<summary><b>Update 3.1</b></summary>
...
<details>
<summary><b>Update 3.2</b></summary>
...
- **Bug Fix - Entlassdatum blieb bei SVWS-Server 1.4 immer leer:** Im DTO `SchuelerSchulbesuchsdaten` wurde das Feld zwischen den SVWS-Versionen umbenannt — `entlassungDatum` (1.3.x) heisst dort jetzt `entlassdatumDieseSchule` (1.4.x). Das Tool las nur den alten Namen, weshalb auf 1.4-Servern **bei jedem** Schüler das Entlassdatum leer blieb, ganz ohne Fehlermeldung (ein fehlendes JSON-Feld liefert schlicht `None`). Betroffen waren dadurch auch die Entlassdatum-Warnungen an die Klassenlehrkräfte, die zweite Ausgabedatei und die Entlassdatum-Spalte der Importdatei. Es werden nun **beide Feldnamen** unterstützt, sodass ältere und neuere SVWS-Server dieselbe Tool-Version nutzen können. Das Entlassdatum der *vorherigen* Schule (`entlassdatumVorherigeSchule`) wird dabei bewusst nicht herangezogen — das ist ein anderer Wert.
- **Neu - Fehlgeschlagene Schulbesuch-Abrufe werden erkannt, gemeldet und optional wiederholt:** Das Entlassdatum muss pro Schüler einzeln abgerufen werden. Schlug ein solcher Abruf fehl — typischerweise, weil der SVWS-Server unter Last mit HTTP 500 antwortet —, wurde der Fehler bisher stillschweigend verschluckt und der betroffene Schüler landete ohne Entlassdatum in der Ausgabe. Das ist jetzt sichtbar: Die Konsole meldet Anzahl und Quote der fehlgeschlagenen Abrufe. Ein *leeres* Entlassdatum zählt dabei ausdrücklich **nicht** als Fehler — während des laufenden Schuljahres ist das der Normalfall. Zwei neue Einstellungen unter `⚙️ Einstellungen → 🏫 Schild API`: **🔁 Wiederholversuche** (`schulbesuch_retries`, Standard `1`, `0` = aus) wiederholt gezielt nur die fehlgeschlagenen Schüler, was bei Lastfehlern meist genügt; **🛑 Abbruch ab Fehlerquote** (`schulbesuch_max_fehlerquote`, Standard `0` = deaktiviert) bricht die API-Verarbeitung ab, sobald zu viele Abrufe scheitern, damit keine lückenhaften Entlassdaten zu falschen Warnungen an Klassenlehrkräfte führen — anschliessend greift der CSV-Fallback.
- **Verbessert - Hinweis, wenn die Option „Abschlussdatum" im SVWS-API-Modus wirkungslos ist:** Die Option leitet das Entlassdatum aus der Spalte `vorauss. Abschlussdatum` ab — entweder als Ersatz, wenn kein Entlassdatum gepflegt ist, oder als früheres der beiden Daten. Da die SVWS-API kein voraussichtliches Abschlussdatum liefert (geprüft gegen SVWS 1.3.1 und 1.4.1: es existiert nur das Flag `istAbschlussPrognose`, kein Datum), lief die Option im API-Modus bisher **stillschweigend** ins Leere. Ist sie aktiv, weist die Konsole jetzt ausdrücklich darauf hin. Im CSV-Modus funktioniert sie unverändert. Die Spalte selbst bleibt bewusst auch im API-Modus in der Ausgabedatei — so ist das Dateiformat unabhängig von der gewählten Datenquelle identisch, und sollte der SVWS-Server das Feld später nachliefern, greift die Option ohne Formatänderung.
- **Neu - Report-Kriterium „Kein Erzieher mit E-Mail" (Erzieher-Workflow):** Der Klassen-Report fehlender Erzieher kannte bisher nur *„Mind. ein Erzieher ohne E-Mail"*. An Schulen, bei denen üblicherweise nur ein Elternteil eine E-Mail-Adresse hinterlegt hat, trifft dieses Kriterium auf nahezu **jeden** Schüler zu — der zweite Erzieher ist zwar mit Namen erfasst, aber ohne Adresse — womit der Report unbrauchbar wird ([Issue #3](https://github.com/CmoneBK/Schild-WebUntis-Tool/issues/3)). Das neue Kriterium schlägt nur an, wenn **kein einziger** hinterlegter Erzieher eine E-Mail hat: Übrig bleiben genau die Schüler, bei denen überhaupt keine Kontaktaufnahme per E-Mail möglich ist. Schüler ganz ohne Erzieher-Daten bleiben dem bestehenden Kriterium *„Kein Erzieher hinterlegt"* vorbehalten, sodass dieselben Schüler nicht in zwei Listen auftauchen. Das alte Kriterium bleibt unverändert erhalten — beide sind frei kombinierbar (ODER/UND).
- **Neu - Feld-Sanity-Check der SVWS-API:** Zwischen SVWS 1.3 und 1.4 wurden im Datensatz `SchuelerSchulbesuchsdaten` 21 von 29 Feldnamen umbenannt — darunter das Entlassdatum (`entlassungDatum` → `entlassdatumDieseSchule`). Weil ein fehlendes JSON-Feld beim Auslesen schlicht „leer" ergibt, blieb die betroffene Spalte danach unbemerkt leer, ohne jede Fehlermeldung. Das Tool prüft die Antworten des Servers jetzt selbst: Liefert er ein Entlassdatum-Feld unter einem unbekannten Namen, oder fehlen erwartete Stammdaten-Felder, erscheint eine deutliche Warnung mit dem tatsächlichen Feldnamen. Damit fällt eine künftige Umbenennung am Tag des Server-Updates auf statt Wochen später über einen Fehlerbericht. Die Prüfung kostet **keine zusätzlichen Abrufe** (sie wertet ohnehin geholte Antworten aus) und meldet sich höchstens einmal pro Lauf. Sie ist bewusst so gebaut, dass ein leeres oder gar nicht vorhandenes Entlassdatum — im laufenden Schuljahr der Normalfall — **keinen** Fehlalarm auslöst.
